...
extern "C" [[noreturn]] void kmain(void) {
    ...
    bool blk_ok = virtio_blk_init(g_blk, g_pmm);
    ...
    if (blk_ok) {
        ...
        ext2_ok = ext2_mount(g_ext2, g_blk, g_pmm);
        if (ext2_ok) {
            qemu_print("ext2 gemountet!\n");
        } else {
            qemu_print("ext2 mount fehlgeschlagen!\n");
        }
    } else {
    ...
    // --- init laden: erst ext2 versuchen, sonst Tar-Initramfs als Fallback ---
    uint32_t init_inode = 0;
    if (ext2_ok) {
        init_inode = ext2_find(g_ext2, "/init");
